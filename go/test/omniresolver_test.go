...
import (
	...
	omni "github.com/voxgig-sdk/gcal-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GcalError); is {
				out := map[string]any{
					"name":    "GcalError",
...
func omniCtx(arg any, client *sdk.GcalSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GcalSDK); is {
			client = live
