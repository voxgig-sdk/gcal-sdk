...
func TestImportEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GCAL_TEST_IMPORT_ENTID JSON to run live")
			return
		}
		// Bootstrap entity data from existing test data (no create step in flow).
		importRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.import")))
		var importRef01Data map[string]any
		if len(importRef01DataRaw) > 0 {
			importRef01Data = core.ToMapAny(importRef01DataRaw[0][1])
		}
...
func importBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"import01", "import02", "import03", "calendar01", "calendar02", "calendar03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GCAL_TEST_IMPORT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GCAL_TEST_IMPORT_ENTID"])
