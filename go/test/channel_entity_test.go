...
func TestChannelEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GCAL_TEST_CHANNEL_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func channelBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GCAL_TEST_CHANNEL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GCAL_TEST_CHANNEL_ENTID"])
