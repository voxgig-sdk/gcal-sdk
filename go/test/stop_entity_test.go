...
func TestStopEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GCAL_TEST_STOP_ENTID JSON to run live")
			return
		}
		// Bootstrap entity data from existing test data (no create step in flow).
		stopRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.stop")))
...
func stopBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GCAL_TEST_STOP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GCAL_TEST_STOP_ENTID"])
