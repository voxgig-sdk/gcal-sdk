...
describe("AclDirect", function()
  it("should direct-list-acl", function()
    local setup = acl_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-acl", setup.live and "live" or "unit")
  ...
  it("should direct-load-acl", function()
    local setup = acl_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-acl", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function acl_direct_setup(mockres)
