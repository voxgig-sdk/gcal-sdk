...
describe('ColorEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    let color_ref01_data = Object.values(setup.data.existing.color)[0] as any

    // LOAD
    const color_ref01_ent = client.Color()
    const color_ref01_match_dt0: any = {}
    const color_ref01_data_dt0 = (await color_ref01_ent.load(color_ref01_match_dt0)).data()
    assert(null != color_ref01_data_dt0)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/color/ColorTestData.json')
