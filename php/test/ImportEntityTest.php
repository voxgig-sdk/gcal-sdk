...
class ImportEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GCAL_TEST_IMPORT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function import_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["import01", "import02", "import03", "calendar01", "calendar02", "calendar03"] as $k) {
    ...
    $entid_env_raw = getenv("GCAL_TEST_IMPORT_ENTID");
