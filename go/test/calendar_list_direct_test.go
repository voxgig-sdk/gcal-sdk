...
func TestCalendarListDirect(t *testing.T) {
	t.Run("direct-list-calendar_list", func(t *testing.T) {
		setup := calendar_listDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-calendar_list", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-calendar_list", func(t *testing.T) {
		setup := calendar_listDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-calendar_list", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type calendar_listDirectSetupResult struct {
	client *sdk.GcalSDK
...
func calendar_listDirectSetup(mockres any) *calendar_listDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GCAL_TEST_CALENDAR_LIST_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GCAL_TEST_CALENDAR_LIST_ENTID"]; ok {
		...
		return &calendar_listDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &calendar_listDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
