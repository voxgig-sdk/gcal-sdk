...
class TestAclDirect:
    ...
    def test_should_direct_list_acl(self):
        setup = _acl_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-acl", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_acl(self):
        setup = _acl_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-acl", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
