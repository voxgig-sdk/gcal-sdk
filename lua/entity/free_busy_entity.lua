-- Gcal SDK FreeBusy entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local FreeBusyEntity = {}
FreeBusyEntity.__index = FreeBusyEntity


function FreeBusyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FreeBusyEntity)
  self._name = "free_busy"
  self._client = client
...
function FreeBusyEntity:get_name()
  return self._name
end


function FreeBusyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FreeBusyEntity.new(self._client, opts)
end
...
function FreeBusyEntity:mark_deleted()
  self._deleted = true
end


function FreeBusyEntity:deleted()
  return true == self._deleted
end


function FreeBusyEntity:data_set(args)
  if args ~= nil then
...
function FreeBusyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FreeBusyEntity:match_set(args)
  if args ~= nil then
...
function FreeBusyEntity:match_get()
...
function FreeBusyEntity:stream(action, args, callopts)
...
---@param reqdata FreeBusyCreateData
---@param ctrl? table
---@return FreeBusy
---@return string? err
function FreeBusyEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function FreeBusyEntity:_run_op(ctx, post_done)
  local utility = self._utility
