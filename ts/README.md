...
`list()` resolves to an array of Acl ENTITIES — every operation
...
```ts
try {
  const acl = await client.Acl().load({
...
```ts
// Create — returns the created Acl ENTITY (.data() for the record)
...
| `scope` | `Record<string, any>` | The extent to which calendar access is granted by this ACL rule. |
...
| `conferenceProperties` | `Record<string, any>` | Conferencing properties for this calendar, for example what types of conferences are allowed. |
...
| `location` | `string` | Geographic location of the calendar as free-form text. |
| `notificationSettings` | `Record<string, any>` | The notifications that the authenticated user is receiving for this calendar. |
...
| Field | Type | Description |
| --- | --- | --- |
| `calendar` | `Record<string, any>` | A global palette of calendar colors, mapping from the color ID to its definition. |
| `event` | `Record<string, any>` | A global palette of event colors, mapping from the color ID to its definition. |
...
| `conferenceData` | `Record<string, any>` | The conference-related information, such as details of a Google Meet conference. |
| `created` | `string` | Creation time of the event (as a RFC3339 timestamp). |
| `creator` | `Record<string, any>` | The creator of the event. |
| `defaultReminders` | `any[]` | The default reminders on the calendar for the authenticated user. |
| `description` | `string` | Description of the event. |
| `end` | `Record<string, any>` | The (exclusive) end time of the event. |
| `endTimeUnspecified` | `boolean` | Whether the end time is actually unspecified. |
| `etag` | `string` | ETag of the resource. |
| `eventType` | `string` | Specific type of the event. |
| `extendedProperties` | `Record<string, any>` | Extended properties of the event. |
| `gadget` | `Record<string, any>` | A gadget that extends this event. |
...
| `organizer` | `Record<string, any>` | The organizer of the event. |
| `originalStartTime` | `Record<string, any>` | For an instance of a recurring event, this is the time at which this event would start according to the recurrence data in the recurring event identified by recurringEventId. |
...
| `reminders` | `Record<string, any>` | Information about the event's reminders for the authenticated user. |
| `sequence` | `number` | Sequence number as per iCalendar. |
| `source` | `Record<string, any>` | Source from which the event was created. |
| `start` | `Record<string, any>` | The (inclusive) start time of the event. |
...
| `workingLocationProperties` | `Record<string, any>` | Developer Preview: Working Location event data. |
...
| `calendars` | `Record<string, any>` | List of free/busy information for calendars. |
| `groupExpansionMax` | `number` | Maximal number of calendar identifiers to be provided for a single group. |
| `groups` | `Record<string, any>` | Expansion of groups. |
