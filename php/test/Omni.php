...
//    only - a typed GcalContext there would make every ctx
...
final class GcalOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new GcalContext($clean, null);
    ...
    public static function err_from_map($m): ?GcalError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new GcalError(is_string($code) ? $code : '', $msg);
    }
