...
describe("CalendarListDirect", function()
  it("should direct-list-calendar_list", function()
    local setup = calendar_list_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-calendar_list", setup.live and "live" or "unit")
  ...
  it("should direct-load-calendar_list", function()
    local setup = calendar_list_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-calendar_list", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function calendar_list_direct_setup(mockres)
