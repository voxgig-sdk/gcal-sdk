...
Calendar API client, generated from the OpenAPI spec.

Manipulates events and other calendar data.

Learn more about Calendar API at [developers.google.com/google-apps/calendar/firstapp](https://developers.google.com/google-apps/calendar/firstapp).

This is an unofficial SDK for the Calendar public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.

Learn more about Voxgig SDKs at [voxgig.com/sdk](https://voxgig.com/sdk/).

Full documentation for this SDK: [https://voxgig-sdk.github.io/gcal-sdk/](https://voxgig-sdk.github.io/gcal-sdk/)
...
const items = await client.Acl().list({ calendar_id: "example" })
```
...
// calendar is the Calendar entity, populated with mock data
...
const client = GcalSDK.test()
const calendar = await client.Calendar().load({ id: 'test01' })
// calendar is the entity, populated with mock data
// — call calendar.data() for the record itself
console.log(calendar)
```
...
| --- | --- | --- |
| TypeScript | `@voxgig-sdk/gcal` | publish pending — [install from git tag](https://github.com/voxgig-sdk/gcal-sdk/tags) |
| Python | `voxgig-sdk-gcal` | publish pending — [install from git tag](https://github.com/voxgig-sdk/gcal-sdk/tags) |
| PHP | `voxgig-sdk/gcal` | publish pending — [install from git tag](https://github.com/voxgig-sdk/gcal-sdk/tags) |
| Golang | `github.com/voxgig-sdk/gcal-sdk/go` | `go get github.com/voxgig-sdk/gcal-sdk/go@latest` |
| Lua | `voxgig-sdk-gcal` | publish pending — [install from git tag](https://github.com/voxgig-sdk/gcal-sdk/tags) |
| JavaScript | `@voxgig-sdk/gcal-js` | publish pending — [install from git tag](https://github.com/voxgig-sdk/gcal-sdk/tags) |
...
// List all acls (returns AclEntity[] — .data() for the record)
const acls = await client.Acl().list({ calendar_id: "example" })
for (const acl of acls) {
  console.log(acl)
}

// Load a specific acl (returns a Acl)
...
| --- | --- | --- |
| **Acl** | The Acl entity (create, list, load, patch, remove, update). | `/calendars/{calendarId}/acl` |
| **Calendar** | The Calendar entity (create, load, patch, remove, update). | `/calendars/{calendarId}` |
| **CalendarList** | The CalendarList entity (create, list, load, patch, remove, update). | `/users/me/calendarList` |
| **Channel** | The Channel entity (create). | `/channels/stop` |
| **Color** | The Color entity (load). | `/colors` |
| **Event** | The Event entity (create, list, load, patch, remove, update). | `/calendars/{calendarId}/events` |
| **FreeBusy** | The FreeBusy entity (create). | `/freeBusy` |
| **Import** | The Import entity. | `` |
| **QuickAdd** | The QuickAdd entity. | `` |
| **Setting** | The Setting entity (create, list, load). | `/users/me/settings` |
| **Stop** | The Stop entity. | `` |
| **Watch** | The Watch entity. | `` |
...
# List all acls (returns a list, raises on error)
acls = client.Acl().list({"calendar_id": "example"})
for acl in acls:
    print(acl)

# Load a specific acl (returns the record, raises on error)
acl = client.Acl().load({"id": "example_id", "calendar_id": "example_calendar_id"})
print(acl)
```
...
// List all acls (returns an array; throws on error)
$acls = $client->Acl()->list();
print_r(array_map(fn($item) => $item->data_get(), $acls));

// Load a specific acl (returns the ENTITY; call data_get() for the record; throws on error)
$acl = $client->Acl()->load(["id" => "example_id", "calendar_id" => "example_calendar_id"]);
...
// Load a specific acl
acl, err := client.Acl(nil).Load(
    map[string]any{"calendar_id": "example_calendar_id", "id": "example_id"}, nil,
)
if err != nil {
    panic(err)
}
...
-- Load a specific acl
local acl, err = client:Acl():load({ id = "example_id", calendar_id = "example_calendar_id" })
print(acl)
```
...
// List all acls (returns an array)
const acls = await client.Acl().list({ calendar_id: "example" })
for (const acl of acls) {
  console.log(acl)
}

// Load a specific acl (returns the entity)
const acl = await client.Acl().load({
  calendar_id: 'example_calendar_id',
  id: 'example_id',
})
...
- Upstream API: [https://google.com](https://google.com)
...
Generated from the Calendar API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
