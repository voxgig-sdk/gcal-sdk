...
func TestAclDirect(t *testing.T) {
	t.Run("direct-list-acl", func(t *testing.T) {
		setup := aclDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-acl", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "calendars/{calendar_id}/acl",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-acl", func(t *testing.T) {
		setup := aclDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-acl", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "calendars/{calendar_id}/acl/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type aclDirectSetupResult struct {
	client *sdk.GcalSDK
...
func aclDirectSetup(mockres any) *aclDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GCAL_TEST_ACL_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GCAL_TEST_ACL_ENTID"]; ok {
		...
		return &aclDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &aclDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
