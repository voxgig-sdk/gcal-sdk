...
4. **Restart Claude Code.** The `gcal_list` and `gcal_load` tools now appear
   in new sessions. Ask the agent to *"list acl using gcal"*
   and it calls `gcal_list` with `{"entity":"acl"}`.
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"gcal_load","arguments":{"entity":"acl","query":{"id":1}}}}'
```
