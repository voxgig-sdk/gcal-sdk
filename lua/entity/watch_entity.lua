-- Gcal SDK Watch entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local WatchEntity = {}
WatchEntity.__index = WatchEntity


function WatchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WatchEntity)
  self._name = "watch"
  self._client = client
...
function WatchEntity:get_name()
  return self._name
end


function WatchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WatchEntity.new(self._client, opts)
end
...
function WatchEntity:mark_deleted()
  self._deleted = true
end


function WatchEntity:deleted()
  return true == self._deleted
end


function WatchEntity:data_set(args)
  if args ~= nil then
...
function WatchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WatchEntity:match_set(args)
  if args ~= nil then
...
function WatchEntity:match_get()
...
function WatchEntity:stream(action, args, callopts)
...
function WatchEntity:_run_op(ctx, post_done)
  local utility = self._utility
