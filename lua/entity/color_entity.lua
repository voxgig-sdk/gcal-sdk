-- Gcal SDK Color entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ColorEntity = {}
ColorEntity.__index = ColorEntity


function ColorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ColorEntity)
  self._name = "color"
  self._client = client
...
function ColorEntity:get_name()
  return self._name
end


function ColorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ColorEntity.new(self._client, opts)
end
...
function ColorEntity:mark_deleted()
  self._deleted = true
end


function ColorEntity:deleted()
  return true == self._deleted
end


function ColorEntity:data_set(args)
  if args ~= nil then
...
function ColorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ColorEntity:match_set(args)
  if args ~= nil then
...
function ColorEntity:match_get()
...
function ColorEntity:stream(action, args, callopts)
...
---@param reqmatch ColorLoadMatch
---@param ctrl? table
---@return Color
---@return string? err
function ColorEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ColorEntity:_run_op(ctx, post_done)
  local utility = self._utility
