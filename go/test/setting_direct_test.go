...
func TestSettingDirect(t *testing.T) {
	...
	t.Run("direct-load-setting", func(t *testing.T) {
		setup := settingDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-setting", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func settingDirectSetup(mockres any) *settingDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"GCAL_TEST_SETTING_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GCAL_TEST_SETTING_ENTID"]; ok {
